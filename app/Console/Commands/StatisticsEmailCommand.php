    ...
    public function handle()
    {
        ...
        foreach (['paid' => 1, 'free' => 0] as $label => $is_paid) {
            ...
            foreach ($team_subscriptions_created_by_week as $result) {
                ...
                $html .= "<li>{$result->name} (team id: {$result->id}, <a href=\"{$impersonateUrl}\">{$owner->email}</a>, {$result->created_at} - {$result->ends_at}, {$result->stripe_status}): {$result->quantity} licenses ({$licenses_used} invited/used, {$payment_method})</li>";
            }
            $html .= "</ul>";
        }
