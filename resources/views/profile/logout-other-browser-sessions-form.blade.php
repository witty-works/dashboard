<x-jet-action-section>
    <x-slot name="title">
        {{ __('content.browser_sessions') }}
    </x-slot>

    <x-slot name="description">
        {{ __('content.manage_and_log_out_your_active_sessions') }}
    </x-slot>

    <x-slot name="content">
        <div class="max-w-xl text-sm text-gray-600">
            {{ __('content.if_necessary_you_may_log_out') }}
        </div>
                            ...
                            <div>
                                <div class="text-xs text-gray-500">
                                    ...
                                    @if ($session->is_current_device)
                                        <span class="text-green-500 font-semibold">{{ __('content.this_device') }}</span>
                                    @else
                                        {{ __('content.last_active') }} {{ $session->last_active }}
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        <div class="flex items-center mt-5">
            <x-jet-button wire:click="confirmLogout" wire:loading.attr="disabled">
                {{ __('content.log_out_other_browser_sessions') }}
            </x-jet-button>
        ...
        <x-jet-dialog-modal wire:model="confirmingLogout">
            ...
            <x-slot name="content">
                {{ __('content.please_enter_your_password_to_confirm') }}
