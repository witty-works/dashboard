<div>
    ...
    <x-form-section submit="createApiToken">
        <x-slot name="title">
            {{ __('content.create_api_token') }}
        </x-slot>

        <x-slot name="description">
            {{ __('content.api_tokens_allow_third_party_services') }}
        </x-slot>

        <x-slot name="form">
            <!-- Token Name -->
            <div class="w-full col-span-6 sm:col-span-4">
                <x-label for="name" value="{{ __('content.token_name') }}" />
            ...
            @if (Laravel\Jetstream\Jetstream::hasPermissions())
                <div class="col-span-6">
                    <x-label for="permissions" value="{{ __('content.permissions') }}" />
    ...
    @if ($this->user->tokens->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-action-section>
                <x-slot name="title">
                    {{ __('content.manage_api_tokens') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('content.you_may_delete') }}
                </x-slot>
    ...
    <x-dialog-modal wire:model.live="displayingToken">
        <x-slot name="title">
            {{ __('content.api_token') }}
        </x-slot>

        <x-slot name="content">
            <div>
                {{ __('content.please_copy_your_new_api_token') }}
            </div>
    ...
    <x-dialog-modal wire:model.live="managingApiTokenPermissions">
        <x-slot name="title">
            {{ __('content.api_token_permissions') }}
        </x-slot>
    ...
    <x-confirmation-modal wire:model.live="confirmingApiTokenDeletion">
        ...
        <x-slot name="content">
            {{ __('content.are_you_sure_api_token') }}
        </x-slot>
